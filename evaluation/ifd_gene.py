...
def parse_args():
    ...
    parser.add_argument("--alg", type=str, default='icl', help='pre, cherry')
...
def main():
    ...
    model.eval()
    args.save_path = args.alg
    args.data_path = f'/data/home/wangwenhao/Enron/generated_data/Med_500_rename/{args.alg}_save_500/client0.json'
    ...
    #     data = json.load(f)
    data = datasets.Dataset.from_json(args.data_path)
    ...
    for i in tqdm(range(len(sampled_data))):
        ...
        temp_data_i = {}
        if args.mod == 'pre':
            ppl_ins_alone, emb_ins_alone = get_perplexity_and_embedding_whole_text(tokenizer, model, instruct_i,
                                                                                   args.max_length)
            ...
            ppl_out_alone, _, loss_list_alone = get_perplexity_and_embedding_part_text(tokenizer, model,
                                                                                       direct_answer_text, output_i,
                                                                                       args.max_length - instruct_i_len + 4)
            ppl_out_condition, _, loss_list_condition = get_perplexity_and_embedding_part_text(tokenizer, model,
                                                                                               whole_text, output_i,
                                                                                               args.max_length)
