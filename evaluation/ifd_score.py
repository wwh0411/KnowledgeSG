...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default='/data/LLMs/llama2')
...
def main():
    ...
    args.pt_data_path = args.alg+'.pt'
    pt_data = torch.load(args.pt_data_path, map_location=torch.device('cpu'))
    # with open(args.json_data_path, "r") as f:
    #     json_data = json.load(f)
    args.json_data_path = f'/data/home/wangwenhao/Enron/generated_data/Med_500_rename/{args.alg}_save_500/client0.json'

    data = datasets.Dataset.from_json(args.json_data_path)
    json_data = [x for x in data]
    ...
    print('Do Rate')
    # print(mean_rate_list)
    def get_avg_idf(mean_rate_list):
        ifd_list = [x[0] for x in mean_rate_list]
        ifd_avg = sum(ifd_list) / len(ifd_list)
        return ifd_avg
    print(get_avg_idf(mean_rate_list))
    exit()
