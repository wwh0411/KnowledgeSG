...
class ScriptArguments:
    ...
    label_names: Optional[str] = field(default=None)
    # for dp training
    use_dp: Optional[bool] = field(default=False, metadata={"help": "Wether to use DP or not to train adapters"})
    v: Optional[bool] = field(default=False)
    peft_model: Optional[str] = field(default=None, metadata={"help": "the peft_model name"})
    

parser = HfArgumentParser((ScriptArguments, FedArguments, dp_transformers.PrivacyArguments))
...
if script_args.use_peft:
    peft_config = LoraConfig(
        ...
        lora_dropout=0.0 if script_args.use_dp else 0.05, # 0.0 for dp
        bias="none",
        task_type="CAUSAL_LM",
    )
else:
...
def get_config():
    ...
    auth_yaml = None
    return script_args, fed_args, privacy_args, peft_config, auth_yaml
...
def get_training_args(script_args, new_lr):
    training_args = TrainingArguments(
        ...
        gradient_checkpointing=script_args.gradient_checkpointing if not script_args.use_dp else False,
        # for dp 
        remove_unused_columns=False if script_args.use_dp else True,
        label_names=script_args.label_names
        # max_grad_norm=0,
...
def save_config(script_args, fed_args, privacy_args):
    ...
    while True:
        if not os.path.exists(output_dir):
            ...
            now_time = (datetime.now(beijing_tz) + timedelta(seconds=1)).strftime("%Y%m%d%H%M%S")
    ...
    with open(os.path.join(script_args.output_dir, "args.json"), "w") as f:
        combined_dict = {
            ...
            "privacy_args": asdict(privacy_args),
        }
